...
export class FilterPokemonPipe implements PipeTransform {
  ...
  transform(pokemonArr: any, args: any): any {
    if (
      !args ||
      (args.searchText === "" && !args.types.length && !args.genders.length)
    ) {
      return pokemonArr;
    }
    return this.filterPokemons(pokemonArr, args).length
      ? this.filterPokemons(pokemonArr, args)
      : [{}];
  }
  ...
  isPokemonExist(filterType: string, pokemon: any, filterParam: any) {
    ...
    switch (filterType) {
      case "search":
        isExist =
          pokemon.name.toLowerCase().includes(filterParam) ||
          pokemon.id.toString().includes(filterParam);
        break;
      case "type":
        isExist = filterParam.length
          ? this.isPokemonTypeExist(pokemon, filterParam)
          : true;
        break;
      case "gender":
        isExist = filterParam.length
          ? this.isPokemonGenderExist(pokemon, filterParam)
          : true;
        break;
    }
    return isExist;
  ...
  isPokemonGenderExist(pokemon: any, filterGenders: any) {
    return (
      filterGenders.filter((gender: any) => {
        return (
          this.pokemonService.genderPokemons[gender.name].indexOf(
            pokemon.name,
          ) > -1
        );
      }).length > 0
